...
class JOVENES_MOVIMIENTO_Controller extends Controller
{
    ...
    public function cargaArchivos(Request $request){
        if($request->file('INE') AND $request->file('COMP_ESTUDIOS')){
            ...
            $ine->storeAs('/documentacion/',$request->FOLIO.'_INE_'.$ine->getClientOriginalName());
            $comp_est->storeAs('/documentacion/',$request->FOLIO.'_COMP-EST_'.$comp_est->getClientOriginalName());
            ...
            $nuevo_doctos->DOCTO_1              = $request->FOLIO.'_INE_'.$ine->getClientOriginalName();
            $nuevo_doctos->DOCTO_2              = $request->FOLIO.'_COMP-EST_'.$comp_est->getClientOriginalName();
...
/*****************************************************************/            
   ...
   public function generarPDF($id){
        ...
        return $pdf->stream('ComprobanteDeRegistro-Folio'.$usuario->folio.'.pdf');
    }
}
